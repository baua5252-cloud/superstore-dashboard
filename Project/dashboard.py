...
fl = st.file_uploader(":file_flder: Upload a file", type=["csv", "xlsx", "xls","txt"])
if fl is not None:
    filename = fl.name
    st.write(filename)
...
# Create a treem based on Region, sub-category
st.subheader("Hierarchical view of sales using Treemap")          
fig3 = px.treemap(filtered_df, path = ["Region", "Sub-Category"], values = "Sales", hover_data = ["Sales"],
                  color = "Sub-Category")
...
st.download_button("Download Dataset", data = csv, file_name = "Dataset.csv", mime = "text/csv",)